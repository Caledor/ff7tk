...
void MateriaEditor::setAP(qint32 ap)
{
    ...
    if (FF7Materia::levels(_id) == 1) {  //Underwater, MasterCommand, MasterMagic, MasterSummon, EnemySkill
...
void MateriaEditor::setName()
{
    if (_id == FF7Materia::EmptyId) {
        ...
        for (int i = 0; i < combo_materia->count(); i++) { //loop thru type and see if name matches if so set index and stop
            if (FF7Materia::name(_id) == combo_materia->itemText(i)) {
...
void MateriaEditor::setSkills()
{
    ...
    if (_id == FF7Materia::EmptyId) {
        ...
        for (int i = 0; i < 8; i++)
            list_skills->addItem(FF7Materia::masterCommandSkill(i));
    } else if(_id == FF7Materia::MasterSummon){
        for (int i = 0; i < 16; i++)
            list_skills->addItem(FF7Materia::masterSummonSkill(i));
    } else if(_id == FF7Materia::MasterMagic){
        for (int i = 0; i < 54; i++)
            list_skills->addItem(FF7Materia::masterMagicSkill(i));
    } else {
        for (int i = 0; i < _level; i++) {
